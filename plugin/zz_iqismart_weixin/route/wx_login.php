 ...
 * https://www.iqismart.com/thread-394.htm
 */
...
if ($action == 'index') {
    ...
    if ($action_2 == 'home') {
        ...
        $username = param('username');
      	$avatar = param('avatar');
        $openid = param('openid');
      	$is_follow = param('is_follow',0);
		if(!$openid) {
            message(-1, '获取 openid 失败，请联系一起smart');
        }else{
        ...
        if (!$user) { 
            //创建新用户
            $user = wx_login_create_user($username, $avatar, $openid, $is_follow);
            if (empty($user)) {
                message($errno, jump('创建新用户失败：' . $errstr, $home_url, 3));
            }
          	bindSuccess($user ['uid']);
        }
		...
		if(!empty($_SESSION['redirect_url'])){
          $home_url = $_SESSION['redirect_url'];
...
//扫描二维码登录
    ...
    if ($action_2 == 'create_qrcode') {
	 ...
	 db_delete('iqismart_wx_login', array('uid' => $user['uid']));
  	 db_update('user',array('uid'=>$user['uid']),array('is_follow'=>0));	
  	message(0,jump('解绑成功',url('my'),3));
}
...
function redirect($param1 = '', $param2 = '') {
    ...
    $link = 'https://www.iqismart.com/iqismart_wx_login-redirect.htm?redirect_uri='.$return_url;
    return $link;
}