...
function bindSuccess($uid){
  $wxlogin = kv_get('iqismart_weixin');
  $bindUser = user_read($uid);
  if($bindUser['bind_gift'] == 0){
   	//未赠送过积分  赠送
    $update_array = array();
    $update_array['rmbs+']=$wxlogin['bind_get_rmbs'];
    $update_array['golds+']=$wxlogin['bind_get_golds'];
    $update_array['credits+']=$wxlogin['bind_get_credits'];
    $update_array['bind_gift']=1;
...
function wx_login_read_user_by_openid($openid) {
    $arr = db_find_one('iqismart_wx_login', array('openid' => $openid));
    if ($arr) {
        $arr2 = user_read($arr['uid']);
        if ($arr2) {
            $arr = array_merge($arr, $arr2);
        } else {
            db_delete('iqismart_wx_login', array('openid' => $openid));
            return FALSE;
        }
    }
    return $arr;
...
function wx_had_bind_user_by_uid($uid) {
    $arr = db_find_one('iqismart_wx_login', array('uid' => $uid));
    if ($arr) {
        return $arr;
    }
    return FALSE;
...
function wx_bind_uid($uid, $openid) {
    ...
    $r = db_insert('iqismart_wx_login', $bind);
    if (empty($r)) {
        return FALSE;
    };
 ...
 * $openid 微信openid
 */
function wx_login_create_user($username, $avatar_url_2, $openid, $is_follow) {
