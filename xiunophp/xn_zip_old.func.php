...
/*
   该类库收集于互联网，版权未知，由 pwndrer4@gmail.com 修正了部分64位下的 bug, 如果谁知道请麻烦告知作者。
*/
...
class php_zip {
	...
	private function addFile($data, $filename, $time = 0) {
		...
		// fixed by pwndrer4@gmail.com 干掉 eval()
	...
	private function read_central_dir($zip, $zipfile) {
		...
		while($pos < $size)
		{
			...
			// fixed 64 bit bug, by pwndrer4@gmail.com
			// 64 bit
			if(PHP_INT_SIZE === 8) {
	...
	// 参考 wordpress 和 https://www.baidu.com/thread-3483.htm
